...
#                   resolution for the DEMs (all DEMs will have the same res)
...
# For each cunck in a project export the DEM to the specified folder
#   using the chunk name as the file name
for chnk in PhotoScan.app.document.chunks:
	filename = folder + '/' + chnk.label + '.tif'
	chnk.exportDem(filename, format="tif", dx=res, dy=res,
        projection=chnk.crs)
	print(filename)
